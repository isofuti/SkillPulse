...
**Проблемы с Netlify:**
- Убедитесь, что все импорты корректны (удалите неиспользуемые импорты типа 'jspdf-font')
- Используйте переменные окружения для API URL вместо хардкода
- Проверьте, что все зависимости установлены в package.json
- Удалите неиспользуемые переменные и импорты (ESLint treats warnings as errors in CI)
- Проверьте все файлы на наличие неиспользуемых переменных: `npm run build`
